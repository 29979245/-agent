...
- [x] 4.1 本地 uvicorn + seed 学生数据，浏览器走通我的页全流程（报告/绑定码/周报弹窗/改密码），验证：无 console 错误、数据与后端返回一致
- [x] 4.2 AI 助教页降级 + mock 流式验证，回归 4-tab 导航与其他 4 页不回归，验证：页面切换与既有页面正常
- [x] 4.3 运行 `pytest tests/ --tb=short -x`（后端未动应全绿）并 `graphify update .` 同步知识图谱，验证：命令通过

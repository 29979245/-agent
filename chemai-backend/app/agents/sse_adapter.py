...
把 LangGraph `astream_events` 事件流转换为 Agent SSE 12 事件：
  phase / tool_call / tool_args / tool_result / text / component / navigate /
  populate / action / exam_images / error / done
...
- 审批暂停（D13）：工具结果含 requires_approval_blocked → 推 awaiting_approval 阶段，
  停止流（不发 done），记录待审批。
